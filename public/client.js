const chartsSection = document.querySelector(".charts");
const progressSection = document.querySelector(".progress");
const readingGradeSection = document.querySelector(".reading-grade");
const readingEaseSection = document.querySelector(".reading-ease");
const sentimentSection = document.querySelector(".sentiment");
const userInput = document.querySelector("[name=user]");
let charts = [];

// Cleanup old charts
function cleanUp() {
  chartsSection.style.display = "none";
  readingGradeSection.innerHTML = "";
  readingEaseSection.innerHTML = "";
  sentimentSection.innerHTML = "";
  charts.forEach(chart => chart.destroy());
  charts = [];
}

// Grab the username from the form, send it to the server and show updates
function getStats() {
  const user = userInput.value;
  if (user.length === 0) return;
  cleanUp();
  userInput.value = "";

  const socket = new WebSocket(`wss://${location.host}/user?name=${user}`);

  socket.addEventListener("open", function(event) {
    progressSection.innerText = "Connected";
  });

  socket.addEventListener("message", function(event) {
    const data = JSON.parse(event.data);
    if ("result" in data) {
      progressSection.innerHTML = "";
      drawCharts(data.result);
      return;
    }
    progressSection.innerText = data.msg;
  });
}

// Render textual analysis charts
function drawCharts(data) {
  if (data.length === 0) {
    progressSection.innerHTML = "No data found for that user.";
    return;
  }
  chartsSection.style.display = "block";

  const readingGradeChart = new ApexCharts(readingGradeSection, {
    series: [
      {
        name: "Reading level",
        data: data.map(article => article.readability.fleschKincaidGrade)
      }
    ],
    chart: {
      height: 400,
      type: "line",
      zoom: {
        enabled: false
      }
    },
    dataLabels: {
      enabled: false
    },
    stroke: {
      curve: "straight",
      colors: ["#3282b8"]
    },
    title: {
      text: "Readability scored as a U.S. grade level.",
      align: "left"
    },
    grid: {
      row: {
        colors: ["#f3f3f3", "transparent"],
        opacity: 0.5
      }
    },
    xaxis: {
      categories: data.map(article => article.title)
    }
  });
  readingGradeChart.render();
  charts.push(readingGradeChart);

  const readingEaseChart = new ApexCharts(readingEaseSection, {
    series: [
      {
        name: "Reading Ease",
        data: data.map(article => article.readability.fleschReadingEase)
      }
    ],
    chart: {
      height: 400,
      type: "line",
      zoom: {
        enabled: false
      }
    },
    dataLabels: {
      enabled: false
    },
    stroke: {
      curve: "straight",
      colors: ["#0f4c75"]
    },
    title: {
      text: "Higher scores indicate the article is easier to read.",
      align: "left"
    },
    grid: {
      row: {
        colors: ["#f3f3f3", "transparent"],
        opacity: 0.5
      }
    },
    xaxis: {
      categories: data.map(article => article.title)
    }
  });
  readingEaseChart.render();
  charts.push(readingEaseChart);

  const sentimentChart = new ApexCharts(sentimentSection, {
    series: [
      {
        name: "Comparative score",
        data: data.map(article => article.sentiment.toFixed(5))
      }
    ],
    chart: {
      height: 400,
      type: "line",
      zoom: {
        enabled: false
      }
    },
    dataLabels: {
      enabled: false
    },
    stroke: {
      curve: "straight",
      colors: ["#1b262c"]
    },
    title: {
      text: "A scale of emotional polarity. Higher is positive.",
      align: "left"
    },
    grid: {
      row: {
        colors: ["#f3f3f3", "transparent"],
        opacity: 0.5
      }
    },
    xaxis: {
      categories: data.map(article => article.title)
    }
  });
  sentimentChart.render();
  charts.push(sentimentChart);
}

// Render example data from username: healeycodes
drawCharts([
  {
    title: "Benchmarking WebSocket Servers with Python!",
    readability: { fleschReadingEase: 24.92, fleschKincaidGrade: 17 },
    sentiment: 0.015570934256055362
  },
  {
    title:
      "Cloning Dominos Order Tracker as a Microservice with Node.js and Firebase!",
    readability: { fleschReadingEase: -41.58, fleschKincaidGrade: 36.4 },
    sentiment: 0.0015267175572519084
  },
  {
    title: "I Ported Sudoku to Your URL Bar and It Plays like a Dream",
    readability: { fleschReadingEase: 14.49, fleschKincaidGrade: 16.9 },
    sentiment: 0.07954545454545454
  },
  {
    title:
      "Real-time streaming of every emoji posted as a comment to Reddit.com 🔥",
    readability: { fleschReadingEase: 16.8, fleschKincaidGrade: 20.2 },
    sentiment: 0.019337016574585635
  },
  {
    title: "Creating Cute GitHub Badges Based Off a Project's Mood (Node.js)",
    readability: { fleschReadingEase: 26.34, fleschKincaidGrade: 16.5 },
    sentiment: 0.03081232492997199
  },
  {
    title:
      "Two Things About My First In-house Software Job That I Didn't See Coming",
    readability: { fleschReadingEase: 52.63, fleschKincaidGrade: 12.6 },
    sentiment: 0.042316258351893093
  },
  {
    title: "Kicking Butt and Taking Names With WebSockets (Tutorial)",
    readability: { fleschReadingEase: 38.76, fleschKincaidGrade: 13.8 },
    sentiment: 0.030439684329199548
  },
  {
    title: "A Day in the Life of a Mediumly Productive Software Engineer",
    readability: { fleschReadingEase: 67.89, fleschKincaidGrade: 8.8 },
    sentiment: 0.034482758620689655
  },
  {
    title: " How Flappy Bird Got Me My Start in Software",
    readability: { fleschReadingEase: 61.26, fleschKincaidGrade: 9.3 },
    sentiment: 0.040880503144654086
  },
  {
    title: "Coding a Live Cam for the Raspberry Pi in Python (Tutorial)",
    readability: { fleschReadingEase: 35.41, fleschKincaidGrade: 15.1 },
    sentiment: 0.023336214347450302
  },
  {
    title:
      "Earn a Build Passing Badge on GitHub ✅! Testing Your Express App with Travis CI",
    readability: { fleschReadingEase: 49.69, fleschKincaidGrade: 13.7 },
    sentiment: 0.011086474501108648
  },
  {
    title:
      "Need a Friend? Write a Reddit Bot with Python to Follow You Around 🤖",
    readability: { fleschReadingEase: 24.82, fleschKincaidGrade: 17.1 },
    sentiment: 0.03225806451612903
  },
  {
    title: "JavaScript One-Liners That Make Me Excited",
    readability: { fleschReadingEase: 37.74, fleschKincaidGrade: 14.2 },
    sentiment: 0.047021943573667714
  },
  {
    title: "Answered: What the Heck is Code Golf? ⛳",
    readability: { fleschReadingEase: 46.1, fleschKincaidGrade: 13 },
    sentiment: 0.056100981767180924
  },
  {
    title: "Build a Python Bot to Find Your Website's Dead Links",
    readability: { fleschReadingEase: 42.34, fleschKincaidGrade: 14.5 },
    sentiment: -0.011764705882352941
  },
  {
    title: "Embed Your Latest DEV Posts Anywhere 💌",
    readability: { fleschReadingEase: 34.29, fleschKincaidGrade: 15.5 },
    sentiment: 0.022972972972972974
  },
  {
    title: "Learn the Storage API by Building a Note Taking App",
    readability: { fleschReadingEase: 33.78, fleschKincaidGrade: 15.7 },
    sentiment: 0.015277777777777777
  },
  {
    title: "Talking to Python from JavaScript (and Back Again!)",
    readability: { fleschReadingEase: 42.85, fleschKincaidGrade: 14.3 },
    sentiment: 0.024918032786885248
  },
  {
    title: "How to Write an Awesome GitHub README",
    readability: { fleschReadingEase: 57.4, fleschKincaidGrade: 10.8 },
    sentiment: 0.08478605388272584
  },
  {
    title: "The Weird Rules I Set Myself That Got Me a Job",
    readability: { fleschReadingEase: 59.43, fleschKincaidGrade: 10 },
    sentiment: 0.048329779673063254
  },
  {
    title: "What They Don't Teach You at a Computer Science Masters",
    readability: { fleschReadingEase: 61.06, fleschKincaidGrade: 9.4 },
    sentiment: 0.06956521739130435
  },
  {
    title: "Fun With Linear Time: My Favorite Algorithm",
    readability: { fleschReadingEase: 38.18, fleschKincaidGrade: 16.1 },
    sentiment: 0.044964028776978415
  },
  {
    title: "Continuous Deployment to Glitch via GitHub Webhooks",
    readability: { fleschReadingEase: 37.03, fleschKincaidGrade: 14.5 },
    sentiment: 0.019402985074626865
  },
  {
    title: "A Tiny Project, From Inception to Deployment",
    readability: { fleschReadingEase: 20.08, fleschKincaidGrade: 21 },
    sentiment: 0.008849557522123894
  },
  {
    title: "Solving Puzzles With High-Performance JavaScript",
    readability: { fleschReadingEase: 62.11, fleschKincaidGrade: 11 },
    sentiment: 0.029900332225913623
  },
  {
    title: "Interview Question: Implement a Progress Bar",
    readability: { fleschReadingEase: 57.61, fleschKincaidGrade: 10.7 },
    sentiment: 0.06666666666666667
  },
  {
    title: "Saving Data in JavaScript Without a Database",
    readability: { fleschReadingEase: 20.18, fleschKincaidGrade: 20.9 },
    sentiment: 0.041501976284584984
  },
  {
    title:
      "Refactoring JavaScript for Performance and Readability (with Examples!)",
    readability: { fleschReadingEase: 22.11, fleschKincaidGrade: 20.2 },
    sentiment: 0.022222222222222223
  },
  {
    title: "Your First Open-Source Pull Request: a Walkthrough",
    readability: { fleschReadingEase: 60.14, fleschKincaidGrade: 9.7 },
    sentiment: 0.023157894736842106
  },
  {
    title: "A Tutorial on Tricking Google: Encoding Messages in Apostrophes",
    readability: { fleschReadingEase: 36.15, fleschKincaidGrade: 16.9 },
    sentiment: 0.016483516483516484
  },
  {
    title: "What If I Want My Website to Last for 100 Years?",
    readability: { fleschReadingEase: 48.94, fleschKincaidGrade: 11.9 },
    sentiment: 0.040289256198347105
  },
  {
    title: "An Introduction to Caching on the Web, With Examples in Python",
    readability: { fleschReadingEase: 44.88, fleschKincaidGrade: 13.5 },
    sentiment: 0.03948576675849403
  },
  {
    title: "My Experience with Pair Programming",
    readability: { fleschReadingEase: 54.86, fleschKincaidGrade: 11.7 },
    sentiment: 0.00902061855670103
  },
  {
    title: "I Built a Bot to Try and Get Money Back From My Internet Provider",
    readability: { fleschReadingEase: 25.87, fleschKincaidGrade: 18.7 },
    sentiment: 0.04264870931537598
  },
  {
    title: "My First Golang Program",
    readability: { fleschReadingEase: 59.03, fleschKincaidGrade: 10.1 },
    sentiment: 0.059726962457337884
  },
  {
    title: "Cloning Memcached with Go",
    readability: { fleschReadingEase: 36.25, fleschKincaidGrade: 16.8 },
    sentiment: 0.014375561545372867
  },
  {
    title: "Let's Build a CAPTCHA Generator with Node.js",
    readability: { fleschReadingEase: 30.16, fleschKincaidGrade: 19.2 },
    sentiment: 0.012829650748396294
  },
  {
    title: "Hardest JavaScript Puzzle I've Ever Solved",
    readability: { fleschReadingEase: 34.16, fleschKincaidGrade: 19.7 },
    sentiment: 0.007368421052631579
  },
  {
    title: "Gatsby Sparks Joy",
    readability: { fleschReadingEase: 43.66, fleschKincaidGrade: 14 },
    sentiment: 0.05950095969289827
  }
]);
